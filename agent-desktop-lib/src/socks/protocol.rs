...
impl From<Socks5Address> for PpaassProtocolAddress {
    fn from(value: Socks5Address) -> Self {
        match value {
            ...
            Socks5Address::Domain(host, port) => PpaassProtocolAddress::Domain { host: host.into_bytes(), port },
        }
    }
}
...
impl From<PpaassProtocolAddress> for Socks5Address {
    fn from(net_addr: PpaassProtocolAddress) -> Self {
        match net_addr {
            ...
            PpaassProtocolAddress::Domain { host, port } => Socks5Address::Domain(String::from_utf8_lossy(&host).to_string(), port),
        }
    }
}
