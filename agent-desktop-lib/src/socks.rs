...
impl From<Socks5Addr> for PpaassProtocolAddress {
    fn from(value: Socks5Addr) -> Self {
        match value {
            Socks5Addr::IpV4(host, port) => PpaassProtocolAddress::IpV4 { ip: host, port },
            Socks5Addr::IpV6(host, port) => PpaassProtocolAddress::IpV6 { ip: host, port },
            Socks5Addr::Domain(host, port) => PpaassProtocolAddress::Domain { host: host.into_bytes(), port },
        }
    }
}

impl From<PpaassProtocolAddress> for Socks5Addr {
    fn from(net_addr: PpaassProtocolAddress) -> Self {
        match net_addr {
            PpaassProtocolAddress::IpV4 { ip, port } => Socks5Addr::IpV4(ip, port),
            PpaassProtocolAddress::IpV6 { ip, port } => Socks5Addr::IpV6(ip, port),
            PpaassProtocolAddress::Domain { host, port } => Socks5Addr::Domain(String::from_utf8_lossy(&host).to_string(), port),
        }
    }
}
