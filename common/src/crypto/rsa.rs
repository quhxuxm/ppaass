...
use crate::{CommonError, RsaError};
...
impl RsaCrypto {
    ...
    pub fn encrypt(&self, target: &[u8]) -> Result<Vec<u8>, RsaError> {
        let result = self.public_key.encrypt(&mut OsRng, Pkcs1v15Encrypt, target.as_ref())?;
        Ok(result)
    }
...
fn generate_rsa_key_pairs(private_key_path: &Path, public_key_path: &Path) -> Result<(), CommonError> {
