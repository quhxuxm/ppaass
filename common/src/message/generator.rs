...
impl PpaassMessageGenerator {
    pub fn generate_tcp_init_request(
        ...
        let message_payload: Vec<u8> = message_payload.try_into()?;
        let message = PpaassMessage::new(generate_uuid(), user_token.to_string(), payload_encryption, message_payload.into());
        Ok(message)
    }
    ...
    pub fn generate_tcp_data(
        ...
        let tcp_data: Vec<u8> = tcp_data.try_into()?;
        let message = PpaassMessage::new(generate_uuid(), user_token.to_string(), payload_encryption, tcp_data.into());
        Ok(message)
    }
